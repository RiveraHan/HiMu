...
type Props = Readonly<{ controller: DjIdentityController; value: DjIdentityDraftValue; disabled?: boolean }>;

export function CreateDjIdentityStep({ controller, value, disabled = false }: Props) {
  ...
  return (
    <PrefSection title={t("dj.identity.title")} subtitle={t("dj.identity.subtitle")}>
      ...
      {controller.candidates.length > 0 ? (
        <View style={styles.candidates} accessibilityRole="radiogroup">
          {controller.candidates.map((candidate) => {
            ...
            return <Pressable key={candidate.name} accessibilityRole="radio" accessibilityLabel={`${candidate.name}. ${candidate.identityConcept}`} accessibilityState={{ selected, disabled }} disabled={disabled} onPress={() => controller.select(candidate)} style={[styles.candidate, selected && styles.candidateSelected]}>
      ...
      <View style={styles.actions}>
        ...
        {controller.selectedName && !editing ? <Button variant="ghost" label="Edit" disabled={disabled} onPress={() => setEditing(true)} /> : null}
      </View>
      ...
      <Button label={t("dj.identity.confirm")} disabled={disabled} onPress={controller.confirm} />
    </PrefSection>
  );
}
