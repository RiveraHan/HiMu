  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "id,user_id,dj_id,status,prompt,generation_brief,source_track_id,error,created_at,updated_at,drop_date,track_id,is_public,djs(id,name),tracks!generation_jobs_track_id_fkey(id,title,artist,audio_url,album_art_url,duration,genre)",
        )
        .eq("user_id", user!.id)
        .is("drop_date", null)
