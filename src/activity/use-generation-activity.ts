  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "id,user_id,dj_id,status,prompt,error,created_at,updated_at,drop_date,track_id,djs(id,name),tracks(id,title,artist,audio_url,album_art_url,duration,genre)",
        )
        .eq("user_id", user!.id)
        .is("drop_date", null)
