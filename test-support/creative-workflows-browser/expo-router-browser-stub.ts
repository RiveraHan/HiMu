...
const ROUTER_EVENT = "himu-create-dj-browser-router-change";
...
export const router = {
  back() {
    window.__HIMU_ROUTER_BACK_COUNT__ =
      (window.__HIMU_ROUTER_BACK_COUNT__ ?? 0) + 1;
    window.history.back();
  },
  canGoBack() {
  ...
  setParams(params: Record<string, string | number | undefined>) {
    ...
    // Expo Router setParams updates the current route; it does not add a
    // synthetic browser-history entry in this fixture.
    window.history.replaceState({}, "", `${url.pathname}${url.search}${url.hash}`);
    window.__HIMU_ROUTER_SET_PARAMS_COUNT__ =
      (window.__HIMU_ROUTER_SET_PARAMS_COUNT__ ?? 0) + 1;
    notify();
  },
};
...
export function useLocalSearchParams<T>() {
  ...
  const params: Record<string, string> = { id: "dj-browser" };
