...
select has_table('public', 'track_moment_cleanup_outbox', 'cleanup outbox exists');
select is((select relrowsecurity from pg_class where oid = 'public.track_moment_cleanup_outbox'::regclass), true, 'cleanup outbox RLS is enabled');
select is(has_table_privilege('authenticated', 'public.track_moment_cleanup_outbox', 'SELECT'), false, 'cleanup targets are not client-readable');
select is(has_table_privilege('service_role', 'public.track_moment_cleanup_outbox', 'SELECT'), true, 'service role can process cleanup targets');
select is(has_function_privilege('authenticated', 'public.queue_track_moment_cleanup(uuid,uuid,uuid,text)', 'EXECUTE'), false, 'authenticated cannot enqueue cleanup directly');
select is(has_function_privilege('service_role', 'public.queue_track_moment_cleanup(uuid,uuid,uuid,text)', 'EXECUTE'), true, 'service role can enqueue cleanup through Edge');
...
select results_eq(
  ...
  $$values ('claimed'::text)$$,
  'a private retry can claim a new operation while prior cleanup is separate'
);
select is(
  public.abort_track_moment_publish(
    '73000000-0000-4000-8000-000000000002',
    '71000000-0000-4000-8000-000000000002',
    '75000000-0000-4000-8000-000000000004'
  ),
  true,
  'abort atomically persists its operation-owned cleanup target'
);
select is(
  (select public_object_key from public.track_moment_cleanup_outbox
    where track_id = '73000000-0000-4000-8000-000000000002'
      and operation_token = '75000000-0000-4000-8000-000000000004'),
  'tracks/generated/job/other.moment-75000000-0000-4000-8000-000000000004.mp3',
  'cleanup target is the exact aborted operation key'
);
select results_eq(
  $$select outcome from public.claim_track_moment_publish(
    '73000000-0000-4000-8000-000000000002',
  ...
  $$values ('claimed'::text)$$,
  'a later publish can proceed without replacing an unresolved cleanup target'
);
select is(
  public.finalize_track_moment_publish(
    '73000000-0000-4000-8000-000000000002',
    '71000000-0000-4000-8000-000000000002',
    '75000000-0000-4000-8000-000000000005',
    'https://media.example/tracks/generated/job/other.moment-75000000-0000-4000-8000-000000000005.mp3',
    'tracks/generated/job/other.moment-75000000-0000-4000-8000-000000000005.mp3'
  ),
  true,
  'a concurrent winner can finalize after the loser aborts'
);
select is(
...
select is(
  ...
  false,
  'the durable winner object can never become a cleanup target'
);
select is(
  public.acknowledge_track_moment_cleanup(
    '73000000-0000-4000-8000-000000000002',
    '71000000-0000-4000-8000-000000000002',
    '75000000-0000-4000-8000-000000000005',
    'tracks/generated/job/other.moment-75000000-0000-4000-8000-000000000005.mp3'
  ...
  false,
  'a winner operation cannot acknowledge a losing cleanup target'
);
select is(
  (select count(*) from public.track_moment_cleanup_outbox
    where track_id = '73000000-0000-4000-8000-000000000002'
      and operation_token = '75000000-0000-4000-8000-000000000004'),
  1::bigint,
  'mismatched acknowledgement leaves the exact target pending'
);
select is(
  public.acknowledge_track_moment_cleanup(
    '73000000-0000-4000-8000-000000000002',
  ...
  true,
  'only verified deletion can acknowledge its own cleanup target'
);
select is(
  (select count(*) from public.track_moment_cleanup_outbox
    where track_id = '73000000-0000-4000-8000-000000000002'
