...
assert.doesNotMatch(sql, /grant all on table public\.product_events to service_role/i);
assert.match(sql, /grant select, insert on table public\.product_events to service_role/i);
...
const appendOnlyNames = readdirSync(resolve("supabase/migrations"))
  .filter((name) => name.endsWith("_himu_product_events_append_only.sql"));
assert.equal(appendOnlyNames.length, 1, "expected one product-events append-only migration");
const appendOnlySql = readFileSync(
  resolve("supabase/migrations", appendOnlyNames[0]!),
  "utf8",
);
assert.match(
  appendOnlySql,
  /revoke all on table public\.product_events from service_role/i,
);
assert.match(
  appendOnlySql,
  /grant select, insert on table public\.product_events to service_role/i,
);
